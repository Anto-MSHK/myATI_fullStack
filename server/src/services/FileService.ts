      ...
      filelinks.map(async link => {
        new Promise<void>(resolve => {
          if (link.url.indexOf('exams') === -1) {
            ...
            //!
            if (link.url.indexOf('spo') === -1 && link.url.indexOf('autumn') === -1) {
              file = fs.createWriteStream(`${basePath}/vpo/${i_vpo}_${link.fileName}.${link.extension}`)
              i_vpo++
            } else if (link.url.indexOf('spo') > -1 && link.url.indexOf('autumn') === -1) {
              file = fs.createWriteStream(`${basePath}/spo/${i_spo}_${link.fileName}.${link.extension}`)
              i_spo++
            } else return
